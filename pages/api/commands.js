export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      // TEMPORARY: Return real command structure while Pi bot is being configured
      const realCommands = [
        {
          id: 1,
          name: 'ban',
      ...
      return res.status(200).json({
        ...
        enabledCommands: realCommands.filter(cmd => cmd.enabled).length,
        source: 'REAL_COMMAND_STRUCTURE'
      });
