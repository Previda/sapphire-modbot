const { SlashCommandBuilder, EmbedBuilder } = require('discord.js');
const fs = require('fs');
const path = require('path');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('move')
        .setDescription('Move a song to a different position in the queue')
        .addIntegerOption(option =>
            option.setName('from')
                .setDescription('Current position of the song')
                .setRequired(true)
                .setMinValue(1))
        .addIntegerOption(option =>
            option.setName('to')
                .setDescription('New position for the song')
                .setRequired(true)
                .setMinValue(1)),

    async execute(interaction) {
        try {
            // Check if music bot is enabled for this server
            if (!checkMusicPermissions(interaction)) {
                return;
            }

            const fromPosition = interaction.options.getInteger('from');
            const toPosition = interaction.options.getInteger('to');
            const guildId = interaction.guild.id;
            
            // Get current music state
            let musicState = getMusicState(guildId);
            if (!musicState || !musicState.queue || musicState.queue.length === 0) {
                const embed = new EmbedBuilder()
                    .setColor('#ed4245')
                    .setTitle('❌ Queue Empty')
                    .setDescription('There are no songs in the queue to move!')
                    .setTimestamp()
                    .setFooter({ text: 'Sapphire Music Bot' });

                return await interaction.reply({ embeds: [embed] });
            }

            const queueLength = musicState.queue.length;

            // Validate positions
            if (fromPosition > queueLength || toPosition > queueLength) {
                const embed = new EmbedBuilder()
                    .setColor('#ed4245')
                    .setTitle('❌ Invalid Position')
                    .setDescription(`Queue only has **${queueLength}** songs. Both positions must be between 1 and ${queueLength}.`)
                    .setTimestamp()
                    .setFooter({ text: 'Sapphire Music Bot' });

                return await interaction.reply({ embeds: [embed] });
            }

            if (fromPosition === toPosition) {
                const embed = new EmbedBuilder()
                    .setColor('#ffa500')
                    .setTitle('⚠️ Same Position')
                    .setDescription('The song is already at that position!')
                    .setTimestamp()
                    .setFooter({ text: 'Sapphire Music Bot' });

                return await interaction.reply({ embeds: [embed] });
            }

            // Move the song
            const song = musicState.queue.splice(fromPosition - 1, 1)[0];
            musicState.queue.splice(toPosition - 1, 0, song);
            saveMusicState(guildId, musicState);

            // Create success embed
            const embed = new EmbedBuilder()
                .setColor('#57f287')
                .setTitle('🔀 Song Moved')
                .setDescription(`**[${song.title}](${song.url})**\n*by ${song.author}*`)
                .addFields(
                    {
                        name: '📍 Position Change',
                        value: `**${fromPosition}** → **${toPosition}**`,
                        inline: true
                    },
                    {
                        name: '🎵 Queue Length',
                        value: `**${queueLength}** songs`,
                        inline: true
                    }
                )
                .setThumbnail(song.thumbnail || null)
                .setTimestamp()
                .setFooter({ text: 'Sapphire Music Bot' });

            // Show updated queue order
            const queuePreview = musicState.queue.slice(0, 8).map((track, index) => {
                const pos = index + 1;
                const marker = pos === toPosition ? '**→**' : '  ';
                return `${marker} ${pos}. **${track.title}** *by ${track.author}*`;
            }).join('\n');

            if (queuePreview.length > 0) {
                embed.addFields({
                    name: '🎶 Updated Queue',
                    value: queuePreview + (queueLength > 8 ? `\n*...and ${queueLength - 8} more*` : ''),
                    inline: false
                });
            }

            await interaction.reply({ embeds: [embed] });

        } catch (error) {
            console.error('Move command error:', error);
            await interaction.reply({
                content: '❌ An error occurred while moving the song.',
                ephemeral: true
            });
        }
    }
};

function checkMusicPermissions(interaction) {
    const settingsPath = path.join(__dirname, '../../data/music-settings.json');
    
    if (!fs.existsSync(settingsPath)) {
        interaction.reply({
            content: '❌ Music bot is not configured for this server. Use `/setup-music` first.',
            ephemeral: true
        });
        return false;
    }

    const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
    const guildSettings = settings[interaction.guild.id];

    if (!guildSettings || !guildSettings.enabled) {
        interaction.reply({
            content: '❌ Music bot is disabled for this server. Ask an admin to enable it with `/setup-music`.',
            ephemeral: true
        });
        return false;
    }

    if (guildSettings.requiredRole) {
        const hasRole = interaction.member.roles.cache.has(guildSettings.requiredRole);
        if (!hasRole && !interaction.member.permissions.has('ADMINISTRATOR')) {
            const role = interaction.guild.roles.cache.get(guildSettings.requiredRole);
            interaction.reply({
                content: `❌ You need the **${role ? role.name : 'required'}** role to use music commands.`,
                ephemeral: true
            });
            return false;
        }
    }

    return true;
}

function getMusicState(guildId) {
    const statePath = path.join(__dirname, '../../data/music-state.json');
    
    if (!fs.existsSync(statePath)) {
        return null;
    }

    const states = JSON.parse(fs.readFileSync(statePath, 'utf8'));
    return states[guildId] || null;
}

function saveMusicState(guildId, state) {
    const statePath = path.join(__dirname, '../../data/music-state.json');
    const dir = path.dirname(statePath);
    
    if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir, { recursive: true });
    }

    let states = {};
    if (fs.existsSync(statePath)) {
        states = JSON.parse(fs.readFileSync(statePath, 'utf8'));
    }

    states[guildId] = state;
    fs.writeFileSync(statePath, JSON.stringify(states, null, 2));
}
